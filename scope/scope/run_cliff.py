...
from alignment.utils import DataCollatorWithPaddingForCLIFF
...
def train_cliff(cfg):
    ...
    training_args = CLIFFArgument(output_dir=output_dir, **training_args)
    #######
    # Setup
    #######
    logging.basicConfig(
    ...
    data_collator = DataCollatorWithPaddingForCLIFF(
        tokenizer,
    ...
    #########################
    # Instantiate cliff trainer
    #########################

    cliff_trainer = CLIFFTrainer(
        model_path,
        model_init_kwargs=model_kwargs,
        args=training_args,
        tau_value=training_args.tau_value,
        lambda_value=training_args.lambda_value,
    ...
    train_result = cliff_trainer.train()
    ...
    cliff_trainer.log_metrics("train", metrics)
    cliff_trainer.save_metrics("train", metrics)
    cliff_trainer.save_state()
    ...
    ##################################
    cliff_trainer.save_model(output_dir)
    ...
    if accelerator.is_main_process:
        ...
        cliff_trainer.model = cliff_trainer.model.original_model
        cliff_trainer.create_model_card(**kwargs)
        # Restore k,v cache for fast inference
        cliff_trainer.model.config.use_cache = True

        cliff_trainer.model.config.save_pretrained(output_dir)
